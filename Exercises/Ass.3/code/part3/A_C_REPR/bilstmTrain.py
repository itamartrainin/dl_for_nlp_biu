...
def learning_tagger(longest_sentence, assignment, train_data, dev_data, device=torch.device("cpu"), num_iterations=10, name_for_outputs="NER", hidden_size=150, lr=1e-1, flag_ner=False):
    ...
    for epoch in range(num_iterations):
        ...
        for batch in range(0, len(train_data), BATCH_SIZE):
            if batch < len(train_data) - BATCH_SIZE:
                ...
                window_idxs = x_all[batch:]
                label_idxs = y_all[batch:]
                prefix_idxs = pre_x_all[batch:]
                suffix_idxs = suf_x_all[batch:]
                char_idxs = char_x[batch:]
                lens_idx = train_lens[batch:]
                num_samples = len(train_data) - batch
            counter += num_samples
            our_model.zero_grad()
            if assignment == "c":
                log_probs = our_model.forward([prefix_idxs, window_idxs, suffix_idxs], lens_idx)
            elif assignment == "b":
                log_probs = our_model.forward(char_idxs, lens_idx)
            elif assignment == "a":
                log_probs = our_model.forward(window_idxs, lens_idx)
            else:
                log_probs = our_model.forward([char_idxs, window_idxs], lens_idx)
            ...
            with torch.no_grad():
                if counter > 499:
                    if assignment == "c":
                        log_probs_dev = our_model.forward([pref_dev, x_dev, suff_dev], dev_lens)
                    elif assignment == "b":
                        log_probs_dev = our_model.forward(char_dev, dev_lens)
                    elif assignment == "a":
                        log_probs_dev = our_model.forward(x_dev, dev_lens)
                    else:
                        log_probs_dev = our_model.forward([char_dev, x_dev], dev_lens)
                    y_hat_dev = torch.argmax(log_probs_dev, dim=2)
                    loss_dev, acc_dev, num_dev_samples = removing_end_calculate_acc_loss(loss_func, log_probs_dev, torch.no_grad, y_hat_dev, y_dev, label_to_ix, flag_ner)
                    dev_loss.append(loss_dev)
                    dev_acc.append(acc_dev * 100 / num_dev_samples)
                    del y_hat_dev
                    counter = 0
        with torch.no_grad():
...
if __name__=="__main__":
    ...
    our_model, label_to_ix, word_to_ix, char_to_ix, pre_to_ix, suf_to_ix, max_word_length, dev_acc, dev_loss = learning_tagger(longest_sentence, assignment, train, dev, device, 15, name_for_outputs, 20, 1e-0, flag_ner)
