...
def read_data(base_dir, task, fname, window_size, freq_bound, balance_coef):
    ...
    return data, word_ix, label_ix, balanced_o_freq, word_count


def generate_X_y(task, data, window_size, word_to_ix, label_to_ix, o_bound, word_count):
    window_delta = int(window_size/2)
    ...
    return X_t.long(), y_t.long()


class Network(nn.Module):
    def __init__(self, task, params, lr, batch_size, iterations):
        ...
        self.windowSize = params[0][0]
        self.inputSize = params[0][1]
        self.embdSize = params[1]
        self.hiddenSize = params[2]
        self.outputSize = params[3]

        # self.E = nn.Embedding(self.inputSize, self.embdSize).cuda()
        # self.hidden = nn.Linear(self.embdSize * self.windowSize, self.hiddenSize).cuda()
        # self.tanh = nn.Tanh().cuda()
        # self.output = nn.Linear(self.hiddenSize, self.outputSize).cuda()
        # self.log_softmax = nn.LogSoftmax(dim=1).cuda()
        #
        # self.loss_function = nn.NLLLoss().cuda()
        # self.optimizer = optim.Adam(self.parameters(), lr=self.lr)

        self.E = nn.Embedding(self.inputSize, self.embdSize)
    ...
    def train_model(self, train, dev):
        ...
        plt.ylabel('accuracy (%) / loss (x10^-3)')
...
if __name__ == '__main__':
    ...
    # base_dir = '/content/drive/My Drive/DL_NLP_Ass_2'

    train, word_to_ix, label_to_ix, o_bound, word_count = read_data(base_dir, task, 'train', window_size, freq_bound, balance_coef)
    X_train, y_train = generate_X_y(task, train, window_size, word_to_ix, label_to_ix, o_bound, word_count)

    dev, _, _, _, _ = read_data(base_dir, task, 'dev', window_size, freq_bound, balance_coef)
    X_dev, y_dev = generate_X_y(task, dev, window_size, word_to_ix, label_to_ix, o_bound, word_count)
    ...
    # model = Network(task, [[window_size, input_size], embd_size, hidden_size, output_size], lr).cuda()
    model = Network(task, [[window_size, input_size], embd_size, hidden_size, output_size], lr, batch_size, iterations)

    model.train_model([X_train, y_train], [X_dev, y_dev])

    # test model
    testf = open(base_dir + '/' + task + '/test', 'r', encoding='utf-8')
    test_outputf = open('test1.' + task, 'w', encoding='utf-8')
