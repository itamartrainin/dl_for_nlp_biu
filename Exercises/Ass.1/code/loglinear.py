...
def softmax(x):
    ...
    """
    e_x = np.exp(x - np.max(x))  # Sub max to get the values negative so if the are too big they go to 0 and not to inf.
    return e_x / e_x.sum()
...
def create_classifier(in_dim, out_dim):
    ...
    # W = np.abs(np.random.randn(in_dim, out_dim))
    # b = np.abs(np.random.randn(out_dim))
    return [W, b]
